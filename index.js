...
// Check if cluster is running as cluster manager mode first
if (cluster.isMaster) {
  // true if cluster is in manager mode, hence need to spin off worker node
  // number of time the fork call is invoked translates to those many instances of node server
  cluster.fork();
  cluster.fork();
} else {
  // runs in worker mode
